...
import { LogOut, User as UserIcon, Shield, Bot, Download, Activity, AlertTriangle, DollarSign, Database, ChevronRight } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const getCounts = () => {
    if (!report?.rawAwsData) return { ec2: 0, s3: 0, rds: 0, lambda: 0, total: 0 };
  ...
  const highPriorityCount = isRuleEngineReport
    ? (summary.findingsBySeverity?.High || 0) + (summary.findingsBySeverity?.Critical || 0)
  ...
  const priorityData = [
    ...
    { name: 'Low', value: isRuleEngineReport ? (summary.findingsBySeverity?.Low || 0) : (ai.priority?.low?.length || 0), color: '#10b981' },
  ].filter(d => d.value > 0);
                        ...
                        <div>
                          <div className="flex items-center mb-2">
                            ...
                            <span className={`text-xs font-semibold ${
                              finding.severity === 'High' || finding.severity === 'Critical' ? 'text-red-400' :
                              finding.severity === 'Medium' ? 'text-amber-400' : 'text-emerald-400'
                            }`}>
                              {finding.severity}
                            </span>
                          </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        <div>
                          <div className="flex items-center mb-2">
                            ...
                            <span className={`text-xs font-semibold ${
                              rec.impact?.toLowerCase() === 'high' ? 'text-red-400' :
                              rec.impact?.toLowerCase() === 'medium' ? 'text-amber-400' : 'text-emerald-400'
                            }`}>
                              {rec.impact} Impact
                            </span>
                          </div>
